...
class AdminCocineroController extends Controller
{
    public function index(): JsonResponse
    {
        $cocineros = Usuario::query()
            ->with(['cargo:idCargo,nombre'])
            ->whereHas('cargo', fn ($q) => $q->where('nombre', 'COCINERO'))
            ->orderByDesc('activo')
            ->orderBy('nombre')
            ->orderBy('apellido')
            ->get();
        ...
        return response()->json([
            'data' => $cocineros->map(fn (Usuario $u) => $this->serializeUsuario($u)),
        ]);
    }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (! $cargoId) {
            return response()->json([
                'message' => 'No existe el cargo COCINERO en el sistema.',
            ], 500);
        }
    ...
    public function update(Request $request, Usuario $usuario): JsonResponse
    {
        ...
        if ($usuario->cargo?->nombre !== 'COCINERO') {
            return response()->json([
                'message' => 'Este endpoint solo permite editar usuarios COCINERO.',
            ], 409);
        }
    ...
    public function setActivo(Request $request, Usuario $usuario): JsonResponse
    {
        ...
        if ($usuario->cargo?->nombre !== 'COCINERO') {
            return response()->json([
                'message' => 'Este endpoint solo permite deshabilitar/habilitar usuarios COCINERO.',
            ], 409);
        }
        ...
        return response()->json([
            'message' => $usuario->activo ? 'Cocinero habilitado.' : 'Cocinero deshabilitado.',
