...
class AdminMeseroController extends Controller
{
    public function index(): JsonResponse
    {
        $meseros = Usuario::query()
            ->with(['cargo:idCargo,nombre'])
            ->whereHas('cargo', fn ($q) => $q->where('nombre', 'MESERO'))
            ->orderByDesc('activo')
            ->orderBy('nombre')
            ->orderBy('apellido')
            ->get();
        ...
        return response()->json([
            'data' => $meseros->map(fn (Usuario $u) => $this->serializeUsuario($u)),
        ]);
    }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $cargoMeseroId = Cargo::query()
            ->where('nombre', 'MESERO')
            ->value('idCargo');

        if (! $cargoMeseroId) {
            return response()->json([
                'message' => 'No existe el cargo MESERO en el sistema.',
            ], 500);
        }
    ...
    public function update(Request $request, Usuario $usuario): JsonResponse
    {
        ...
        if ($usuario->cargo?->nombre !== 'MESERO') {
            return response()->json([
                'message' => 'Este endpoint solo permite editar usuarios MESERO.',
            ], 409);
        }
    ...
    public function setActivo(Request $request, Usuario $usuario): JsonResponse
    {
        ...
        if ($usuario->cargo?->nombre !== 'MESERO') {
            return response()->json([
                'message' => 'Este endpoint solo permite deshabilitar/habilitar usuarios MESERO.',
            ], 409);
        }
        ...
        return response()->json([
            'message' => $usuario->activo ? 'Mesero habilitado.' : 'Mesero deshabilitado.',
